        ...
        if (otherUserId === 'SUPPORT') {
            ...
            query = `
                ...
                WHERE (sender_id = $1 AND receiver_id IS NULL)
                   OR (receiver_id = $1 AND sender_id IN (SELECT id FROM users WHERE role IN ('Admin', 'Manager')))
            ...
            query = `
                SELECT * FROM chat_messages 
                WHERE (sender_id = $1 AND (receiver_id IS NULL OR receiver_id IN (SELECT id FROM users WHERE role IN ('Admin', 'Manager'))))
                   OR (receiver_id = $1 AND sender_id IN (SELECT id FROM users WHERE role IN ('Admin', 'Manager')))
                ORDER BY created_at ASC
            `;
            params = [otherUserId];
    ...
    try {
        const result = await pool.query(
            `WITH last_messages AS (
                SELECT 
                    CASE 
                        WHEN sender_id IN (SELECT id FROM users WHERE role IN ('Admin', 'Manager')) THEN receiver_id 
                        ELSE sender_id 
                    END as customer_id,
                    message,
                    created_at,
            ...
            WHERE lm.rn = 1 AND u.id IS NOT NULL 
              AND u.role NOT IN ('Admin', 'Manager') -- Only show customers in the support list
