...
export function useOrders() {
  const syncOrderStatusFromSocket = async (payload: {
    orderId?: number;
    status?: Order["status"];
  }) => {
    ...
    try {
      ...
      orders.value = data.map((o: any) => ({
        ...
        completedTime: o.completed_time
          ? new Date(o.completed_time).toLocaleString()
          : undefined,
        ...
        items: o.items.map((i: any) => ({
          ...
          productImage: i.image_url?.startsWith("/")
            ? `${API_URL.replace("/api", "")}${i.image_url}`
            : i.image_url,
          quantity: i.quantity,
  ...
  const updateOrderStatus = async (
    ...
    try {
      ...
      if (order) {
        ...
        if (cancelReason)
          order.status === "Cancelled" && (order.cancel_reason = cancelReason);
      }
    } catch (err) {
